...
def partition_nd(columns, filter_value, possible_values):
    ...
    * returns a 1d array of lists of indices
    ...
    for idx in np.ndindex(*size):
        ...
        # for the usual case (filter + 1 dimension -- period does not count),
        # it is faster as soon as we have more than ~50000 rows, which will
        # probably be the usual case.
    ...
    # category it belongs to. python built-in bisect is faster (average time
    # on all indexes) than list.index() starting from ~20 elements. We usually
    # have even less elements than that though :(.
    # Strangely bisect on a list is 2x faster than np.searchsorted on an array,
    # even with large array sizes (10^6).
...
#                pos = tuple([np.searchsorted(values_i, vi) for vi, values_i
