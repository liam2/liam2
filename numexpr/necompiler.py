...
def NumExpr(ex, signature=(), copy_args=(), **kwargs):
    ...
    """
    # This should not happen if NumExpr is called from evaluate (since 
    # truediv='auto' is already translated in that case).
    if kwargs.get('truediv', False) == 'auto':
        f_globals = sys._getframe(1).f_globals
        kwargs['truediv'] = \
            f_globals.get('division', None) == __future__.division
...
def evaluate(ex, local_dict=None, global_dict=None, **kwargs):
    ...
    f_globals = call_frame.f_globals
    ...
    try:
        ...
        compiled_ex = _numexpr_cache[numexpr_key] = \
                      NumExpr(ex, signature, copy_args, **context)
